            ...
            (
                ...
                $$"""
                  ...
                  {{
                      ...
                      public static TResult Match{{allGenericsWithResultStr}}({{interfaceName}}{{allGenericsStr}} {{basicNameFirstLowered}}, {{string.Join(",", choiceInfos.Select(x => $"Func<{x.TypeValueParameterTypeNameStr}, TResult> func{x.ChoiceName}"))}}){{allConstraintsStr}} =>
                          {{basicNameFirstLowered}}.Kind switch
                          {
                  {{
                      string.Join("\n", choiceInfos.Select(x => $"            {enumSymbolNameWithGlobal}.{x.ChoiceName} => func{x.ChoiceName}(GetValue(({x.TypeNameStr}{allGenericsStr}){basicNameFirstLowered})),"))
                  }}
                              _ => throw new Exception("Enum value not handled")
                          };
                  
                      public static System.Threading.Tasks.Task<TResult> MatchAsync{{allGenericsWithResultStr}}({{interfaceName}}{{allGenericsStr}} {{basicNameFirstLowered}}, {{string.Join(",", choiceInfos.Select(x => $"Func<{x.TypeValueParameterTypeNameStr}, System.Threading.Tasks.Task<TResult>> func{x.ChoiceName}"))}}){{allConstraintsStr}} =>
                          {{basicNameFirstLowered}}.Kind switch
                          {
                  {{string.Join("\n", choiceInfos.Select(x => $"            {enumSymbolNameWithGlobal}.{x.ChoiceName} => func{x.ChoiceName}(GetValue(({x.TypeNameStr}{allGenericsStr}){basicNameFirstLowered})),"))}}
                              _ => throw new Exception("Enum value not handled")
